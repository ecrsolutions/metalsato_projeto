		...
		If MsSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)
			...
			Else
	          ...
	          // Posiciono na TES do cupom fiscal para pegar a natureza de operação da nf sobre cupom
						...
						If SA1->A1_XPED == "S" .And. !Empty(SC6->C6_PEDCLI) //.And. !Empty(SC6->C6_XLINPED)       // Se há obrigatoriedade para definição da tag xPed
							//If SF4->F4_DUPLIC == "S"   // Somente gera essa tag caso o item gere financeiro
								aPedCom[Len(aPedCom)] := { SC6->C6_PEDCLI, SC6->C6_XLINPED}
							//Endif
						Endif
...
//  SATO - 14/01/2020 - INCLUÍDA TRANSMISSÃO DO D2_PRCVEN
cString += '<vUnTrib>'+ IIf(cF2Tipo == "C",ComplPreco(cTipo,cF2Tipo,aProd),ConvType(aProd[16],21,8))+'</vUnTrib>'	
