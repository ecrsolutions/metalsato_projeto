...
Static Function RunProc(mv_par01)
	...
	For x:=1 To Len(aImport)
					...
					If SB6->(dbSeek(XFILIAL("SB6")+SD1->(D1_COD+D1_FORNECE+D1_LOJA+D1_IDENTB6)))
						...
						If aSaldo[nPos,2] >= nQuant
							...
							If Round(nPreco,2) == Round(SB6->B6_PRUNIT,2)
								...
								AAdd( aExcel , { "S", AllTrim(aImport[x,nPPrd]), Trim(SB1->B1_COD), Trim(SB1->B1_DESC), nQuant, nPreco, AllTrim(aImport[x,nPDoc]), "OK."})
							Else
		...
		If !Empty(cTpoErro)  // Se não encontrou erro
			...
			nQuant, nPreco, AllTrim(aImport[x,nPDoc]), "ERRO: "+cTpoErro})
		Endif
	Next
...
Static Function Planilha(aVetor)
	...
	@ 190,05 SAY "Total de itens lidos" SIZE 80,8 OF oPanelT PIXEL FONT oFnt1 COLOR CLR_BLUE
	
	@ 190,95 SAY oTot VAR Transform(Len(aVetor),"@E 999,999") PIXEL OF oPanelT FONT oFnt1 SIZE 85,10 COLOR CLR_HRED
...
¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦
¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯*/
User Function STGeraPedido(vPedido,lMonitor)
	Local x, y, nValor, cMatchID
	...
	For x:=1 To Len(vPedido)
		...
		If lMonitor
			StaticCall(STFATA01,RefreshMonitor,"1",{ "1" , cNumPed, "", "", "Gerando pedido de Retorno Simbólico na filial "+cFilAnt+"..."})
		Endif
		...
		For y:=1 To Len(vPedido[x,4])
			...
			aAdd(aArrSC6, {{ "C6_FILIAL"  , cFilSC6             , Nil},;
							{ "C6_NUM"     , cNumPed             , Nil},;
							{ "C6_ITEM"    , cItem               , Nil},;
							{ "C6_PRODUTO" , SB1->B1_COD         , Nil},;
							{ "C6_DESCRI"  , SB1->B1_DESC        , Nil},;
							{ "C6_QTDVEN"  , vPedido[x,4][y,3]   , Nil},;
							{ "C6_ENTREG"  , dDataBase           , Nil},;
							{ "C6_UM"      , SB1->B1_UM          , Nil},;  // { "C6_SEGUM"   , SB1->B1_SEGUM       , Nil},;
							{ "C6_TES"     , vPedido[x,5]        , Nil},;
							{ "C6_CF"      , SF4->F4_CF          , Nil},;  // { "C6_XSTATUS" , "S"                 , Nil},;
							{ "C6_QTDLIB"  , vPedido[x,4][y,3]   , Nil},;
							{ "C6_LOCAL"   , SB1->B1_LOCPAD      , Nil},;
							{ "C6_CLI"     , SA1->A1_COD         , Nil},;
							{ "C6_LOJA"    , SA1->A1_LOJA        , Nil},;
							{ "C6_OP"      , "02"                , Nil},;
							{ "C6_TPOP"    , "F"                 , Nil},;
							{ "C6_PRUNIT"  , vPedido[x,4][y,4]   , Nil},;
							{ "C6_PRCVEN"  , vPedido[x,4][y,4]   , Nil},;
							{ "C6_VALOR"   , nValor              , Nil},;
							{ "C6_NFORI"   , SD1->D1_DOC         , Nil},;
							{ "C6_SERIORI" , SD1->D1_SERIE       , Nil},;
							{ "C6_ITEMORI" , SD1->D1_ITEM        , Nil},;
							{ "C6_IDENTB6" , vPedido[x,4][y,2]   , Nil},;
							{ "C6_PEDCLI"  , vPedido[x,4][y,6]   , Nil},;
							{ "C6_XLINPED" , vPedido[x,4][y,7]   , Nil},;
							{ "C6_XMATCH"  , cMatchID            , Nil}})
		Next
		...
		Else
			...
			If lMonitor
				StaticCall(STFATA01,RefreshMonitor,"3","Erro na geração do pedido de Retorno Simbólico !")
			Endif
		EndIf
	Next
