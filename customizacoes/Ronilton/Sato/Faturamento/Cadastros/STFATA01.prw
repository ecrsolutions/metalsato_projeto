...
Static Function Gravacao()
	...
	If !lMsErroAuto
		GeraPedido(cFilInd,aSC6Ind,cNaturez,"I",cFilInd)
	Endif
	...
	If !lMsErroAuto
		...
		If u_STFATE01(SF2->F2_DOC,SF2->F2_SERIE)
...
Static Function GeraPedido(cFilAtu,aItens,cNaturez,cBase,cFilCli)
	Local cNumPed
	Local aArray  := {}
	Local aPvlNfs := {}
	Local cMens   := If( cBase == "I" , "Venda", "Transferencia")
	...
	RefreshMonitor("1",{ "1" , cNumPed, "", "", "Gerando pedido de "+cMens+" na filial "+cFilAnt+"..."})
	...
	If lMsErroAuto
		RefreshMonitor("3","Erro na geração do pedido de " + cMens + " !")
...
User Function STLiberaPedido(cMens,aPvlNfs)
	...
	RefreshMonitor("1","Liberando pedido de " + cMens + "...")
	...
	If Empty(aBloqueio) .And. !Empty(aPvlNfs)
		RefreshMonitor("2","Pedido de " + cMens + " gravado com sucesso !")
	Else
		RefreshMonitor("3","Ocorreu problema na liberação do pedido de " + cMens)
		lMsErroAuto := .T.
...
Static Function AdicionaItem(cFilAtu,aArray,cBase,nQuant)
	...
	nPreco  := StaticCall(STFATP01,fPrecoTab,SB1->B1_COD,SA1->A1_COD+SA1->A1_LOJA)
...
User Function STGeraSaida(cBase,cMens,aPvlNfs)
	...
	RefreshMonitor("1",{ "1" , "", cNumero, cSerie, "Gerando nota de "+cMens+" na filial "+cFilAnt+"..."} )
	...
	If lMsErroAuto
		RefreshMonitor("3","Ocorreu um erro na geração da nota de " + cMens)
	Else
		RefreshMonitor("2","Nota Fiscal de " + cMens + " gerada com sucesso.")
...
User Function STTransmissao(cLegenda)
	...
	RefreshMonitor("1",{ "1" , "", SF2->F2_DOC, SF2->F2_SERIE, "Transmitindo nota de "+cLegenda+"..."} )
	...
	If cStatus == "2" .And. MsgYesNo("Deseja imprimir a DANFE da nota " + SF2->F2_DOC+" / "+SF2->F2_SERIE+" ?","Impressão DANFE")
		...
		//StaticCall(SAESTA05,GetF2FIMP,SF2->F2_FILIAL+SF2->F2_DOC+SF2->F2_SERIE)
	Endif
...
Static Function GeraEntrada(cFilAtu)
	...
	RefreshMonitor("1",{ "1" , "", cDocNFS, cSerNFS, "Gerando nota de entrada na filial "+cFilAtu+"..."} )
	...
	cFilAnt := cFilAtu
	cLocExp := Alltrim(GetMV("MV_XLOCEXP",.F.,"02"))
	cTES    := GetMV("MV_XESP005",.F.,"001")    // TES padrão de entrada
	...
	If lMsErroAuto
		RefreshMonitor("3","Ocorreu um erro na geração da nota de entrada")
	Else
		RefreshMonitor("2","Nota Fiscal de entrada gerada com sucesso.")
	Endif
	
Return !lMsErroAuto

Static Function RefreshMonitor(cStatus,xMens)
...
Static Function CalcSaldoEntrega(cProduto)
	...
	While !SZ2->(Eof()) .And. XFILIAL("SZ2")+cChave == SZ2->Z2_FILIAL+SZ2->Z2_CLIENTE+SZ2->Z2_LOJA+SZ2->Z2_PRODUTO+DTOS(SZ2->Z2_DATENT)
		nRet += SZ2->Z2_QUANT - SZ2->Z2_QTDENT
		SZ2->(dbSkip())
	Enddo
...
Static Function Marcar(cMarca,cProduto,cOrigem,lMarcado)
	...
	If nEnt == 0
		...
		SZ2->(dbSeek(XFILIAL("SZ2")+cChave,.T.))
		While !SZ2->(Eof()) .And. XFILIAL("SZ2")+cChave == SZ2->Z2_FILIAL+SZ2->Z2_CLIENTE+SZ2->Z2_LOJA+SZ2->Z2_PRODUTO+DTOS(SZ2->Z2_DATENT)
			If SZ2->Z2_QTDENT < SZ2->Z2_QUANT    // Adiciona somente os registros com saldo
				AAdd( aEntregas , { SZ2->Z2_PRODUTO, SZ2->Z2_DATENT, SZ2->Z2_HORENT, SZ2->Z2_SETENT, SZ2->Z2_QUANT - SZ2->Z2_QTDENT, 0, {}} )
			Endif
			SZ2->(dbSkip())
		Enddo
	Endif
...
User Function STGravaKanban(lAdiciona,lDel,cKey)
	...
	Local aSoma   := If( lDel == Nil .Or. !lDel , SomaKanban(), {0,0})
	...
	If (cAlias2)->(dbSeek(XFILIAL(cAlias2)+cChave))
		...
		If lDel    // Se for exclusão
			dbDelete()
		Else
			(cAlias2)->Z2_QUANT  := aSoma[1]
			//(cAlias2)->Z2_QTDENT := aSoma[2]
			
			If (cAlias2)->Z2_QUANT <= 0 .And. (cAlias2)->Z2_QTDENT <= 0    // Exclui registro caso não tenha quantidades válidas
				dbDelete()
			Endif
		Endif
		MsUnLock()
...
Static Function SomaKanban()
	...
	While !SZ1->(Eof()) .And. SZ1->Z1_FILIAL+SZ1->Z1_CLIENTE+SZ1->Z1_LOJA+SZ1->Z1_PRODUTO+DTOS(SZ1->Z1_DATENT)+SZ1->Z1_HORENT+SZ1->Z1_SETENT == cKey
		nQtd += If( SZ1->Z1_ATUKANB == "1" , SZ1->Z1_QUANT, 0)
		nEnt += SZ1->Z1_QTDENT
		SZ1->(dbSkip())
	Enddo
	SZ1->(dbSetOrder(nInd))
