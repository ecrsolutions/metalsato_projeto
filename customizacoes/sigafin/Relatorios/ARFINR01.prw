...
ฑฑณPrograma  ณ ARFINR01ณ Autor ณ Marcel R. Grosselli    ณ Data ณ 11/12/15 ณฑฑ
...
฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿
/*/
User Function ARFINR01 (cPrefixo,cNumero)
...
wnrel    := "ARFINR01"
lEnd     := .F.
cPerg    := PADR("ARFINR01",Len(SX1->X1_GRUPO))
...
While !EOF()
   ...
   While !EOF() .And. cDocumen == E1_PREFIXO+E1_NUM+E1_CLIENTE+E1_LOJA
      ...
      aDadosTit := {E1_NUM+If(Empty(E1_PARCELA),"","-"+E1_PARCELA)+;
                    ...
                    SE1->E1_SALDO-(nVlrAbat+SE1->E1_DECRESC),;  // [5] Valor do tํtulo
      ...
      aBolText    := {"","","","",""}
   	  aBolText[1]:="JUROS DIARIO DE: R$ "+SUBSTR(AllTrim(Transform(E1_SALDO * GETMV("MV_XJURBOL")/100,"@E 9,999,999.99")),1,13)+" A PARTIR DO DIA: "+SUBSTR(DTOC(E1_VENCTO+1),1,10)
      aBolText[2]:= IIF(!EMPTY(E1_PEDIDO),"REFERENTE AO PEDIDO NฺMERO: "+E1_PEDIDO,"") 
      aBolText[3]:= "Sujeito ao Serasa apos 15 dias do vencimento"
      aBolText[4]:= "Santa Maria, estrela da salva็ใo, que eu chegue ao porto desejado"
      aBolText[5]:= "" //reconhecidas por este sistema. Nใo cessa esta cobran็a, podendo ocorrer protesto."      
...
cLogo := cStartPath + "LOGOARTRIN.BMP" //Logo da Amazon A็o   
...
oPrint:Say  (nRow1+0150,1060,"Ag๊ncia / C๓digo do Beneficiแrio",oFont8)
...
oPrint:Say  (nRow1+0350,0100,"Recebi(emos) o bloqueto/tํtulo",oFont10)
oPrint:Say  (nRow1+0400,0100,"com as caracterํsticas acima.",oFont10)
...
oPrint:Say  (nRow1+0190,100 ,"ATษ O VENCIMENTO, PREFERENCIALMENTE NO ITAฺ. APำS O VENCIMENTO, SOMENTE NO ITAฺ",oFont9)
...
oPrint:Say  (nRow1+0290,100 ,"Endere็o ",oFont8n)
...
oPrint:Say  (nRow1+0250,1810,"Ag๊ncia / C๓digo do Beneficiแrio",oFont8n)
...
vMens[2] := "Esta quita็ใo s๓ terแ validade ap๓s o pagamento do cheque pelo banco sacado."
...
oPrint:Say  (nRow3+0190,100 ,"ATษ O VENCIMENTO, PREFERENCIALMENTE NO ITAฺ. APำS O VENCIMENTO, SOMENTE NO ITAฺ",oFont9)
...
oPrint:Say  (nRow3+0250,1810,"Ag๊ncia / C๓digo do Beneficiแrio",oFont8n)
...
oPrint:Say  (nRow3+1030,1580,"Autentica็ใo Mecโnica "  ,oFont8n)
