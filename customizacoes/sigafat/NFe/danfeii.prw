...
// SATO - 10/01/2020 - ALTERAÇÃO PARA TRUNCAR VALORES NA IMPRESSÃO
Private __aDecQtd := TamSX3("D2_QUANT")
Private __aDecPrc := TamSX3("D2_PRCVEN")
...
If MV_PAR04 == 2
	...
	If SA1->(FieldPos("A1_XDECQTD")) > 0 .And. SA1->A1_XDECQTD > 0
		__aDecQtd[2] := SA1->A1_XDECQTD
	Endif
	If SA1->(FieldPos("A1_XDECPRC")) > 0 .And. SA1->A1_XDECPRC > 0
		__aDecPrc[2] := SA1->A1_XDECPRC
	Endif
Endif
...
// SATO - 14/01/2020 - IMPRIMIR O CODIGO DO CLIENTE E O PEDIDO DO CLIENTE
aItemPego := {}
nTamProd  := TamSX3("D2_COD")[1]
cSeekSD2  := SD2->(XFILIAL("SD2"))+SF2->F2_DOC+SF2->F2_SERIE+SF2->F2_CLIENTE+SF2->F2_LOJA
...
For nZ := 1 To nLenDet
	...
	// SATO - 14/01/2020 - IMPRIMIR O CODIGO DO CLIENTE E O PEDIDO DO CLIENTE
	cProduto := PADR(oDet[nX]:_Prod:_cProd:TEXT,nTamProd)
	cQtdNota := StrZero(nQtd*__aDecQtd[2],14)
	cComplem := ""
	
	SD2->(dbSeek(cSeekSD2+cProduto,.T.))
	While !SD2->(Eof()) .And. SD2->D2_FILIAL+SD2->D2_DOC+SD2->D2_SERIE+SD2->D2_CLIENTE+SD2->D2_LOJA+SD2->D2_COD == cSeekSD2+cProduto
		
		If AScan( aItemPego , {|x| x[1]+x[2]/*+x[3]*/ == SD2->D2_COD+SD2->D2_ITEM/*+cQtdNota*/ }) == 0
			// Posiciona no Iem do Pedido
			SC6->(dbSetOrder(1))
			SC6->(dbSeek(xFilial("SC6") + SD2->D2_PEDIDO + SD2->D2_ITEMPV + SD2->D2_COD ))
			// Posiciona no Cliente x Produto
			SA7->(dbSetOrder(1))
			If SA7->(dbSeek(xFilial("SA7") + SD2->D2_CLIENTE + SD2->D2_LOJA + SD2->D2_COD )) .And. !Empty(SA7->A7_CODCLI)
				cComplem := "Cod.Cliente: " + Trim(SA7->A7_CODCLI)
			Endif
			If !Empty(SC6->C6_PEDCLI)
				cComplem += If(Empty(cComplem), "", " - ") + "Ped.Cliente: " + Trim(SC6->C6_PEDCLI)
			Endif
			If !Empty(SC6->C6_NFORI)
				cComplem += If(Empty(cComplem), "", " - ") + "Nf.Ori: " + Trim(SC6->C6_NFORI)
			Endif

			AAdd( aItemPego , { SD2->D2_COD, SD2->D2_ITEM, cQtdNota} )
			Exit
		Endif
	...
	aAux := {}
	AADD(aAux, AllTrim(TransForm(nQtd,TM(nQtd,__aDecQtd[1],__aDecQtd[2]))))    // SATO - 10/01/2020 - ALTERAÇÃO DECIMAL
	AADD(aAux, AllTrim(TransForm(nVUnit,TM(nVUnit,__aDecPrc[1],__aDecPrc[2]))))    // SATO - 10/01/2020 - ALTERAÇÃO DECIMAL
	...
	If ( oDet[nX]:_Prod:_uTrib:TEXT <> oDet[nX]:_Prod:_uCom:TEXT )
		...
		aAuxCom := {}
		AADD(aAuxCom, AllTrim(TransForm(nQtdTrib,TM(nQtdTrib,__aDecQtd[1],__aDecQtd[2]))))    // SATO - 10/01/2020 - ALTERAÇÃO DECIMAL
		AADD(aAuxCom, AllTrim(TransForm(nVUnitTrib,TM(nVUnitTrib,__aDecPrc[1],__aDecPrc[2]))))    // SATO - 10/01/2020 - ALTERAÇÃO DECIMAL
	...
	// SATO - 14/01/2020 - IMPRIMIR O CODIGO DO CLIENTE E O PEDIDO DO CLIENTE
	If lImpInfAd
		cComplem := oDet[nX]:_Infadprod:TEXT
	Endif

	// SATO - 14/01/2020 - IMPRIMIR O CODIGO DO CLIENTE E O PEDIDO DO CLIENTE
	If (ValAtrib("oNf:_infnfe:_det[nPrivate]:_Infadprod:TEXT") <> "U" .Or. ValAtrib("oNf:_infnfe:_det:_Infadprod:TEXT") <> "U") .And. ( lImpAnfav .Or. lImpInfAd ) .Or. (!lImpInfAd .And. !Empty(cComplem))
		If at("<", AllTrim(SubStr(cComplem,1))) <> 0
			cAux := stripTags(AllTrim(SubStr(cComplem,1)), .T.) + " "
			cAux += stripTags(AllTrim(SubStr(cComplem,1)), .F.)
		else
			cAux := stripTags(AllTrim(SubStr(cComplem,1)), .T.)
		endIf
...
// SATO - 23/01/2020 - INCLUSÃO DO CODIGO DE BARRAS COM O NÚMERO DA NOTA
oDanfe:Code128(12,390,StrZero(Val(oIdent:_NNf:Text),9), 1, 23 , .F. )
