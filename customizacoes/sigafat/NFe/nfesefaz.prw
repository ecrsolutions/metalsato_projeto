			...
			Else
				...
				While !Eof() .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
					...
					If lContinua
						...
						//>>>>  SATO - 10/01/2020 - INCLUSÃO DE NÚMERO DO xPed  <<<<<
						cPedCli := If(SA1->(FieldPos("A1_XINFRET"))>0.And.SA1->A1_XINFRET=="2".and.!empty(SC6->C6_NFORI),SC6->C6_NFORI,SC6->C6_PEDCLI)
						If SA1->A1_XPED == "S" .And. !Empty(cPedCli) //.And. !Empty(SC6->C6_XLINPED)       // Se há obrigatoriedade para definição da tag xPed
							//If SF4->F4_DUPLIC == "S"   // Somente gera essa tag caso o item gere financeiro
								aPedCom[Len(aPedCom)] := { cPedCli, SC6->C6_XLINPED}
							//Endif
						Endif
						...
						cTPNota := If( Empty(cTPNota) , NfeTpNota(aNota,aNfVinc,cVerAmb,aNfVincRur,aRefECF,cD2Cfop), cTPNota)   // SATO - 14/10/2021 - Ronilton - Pegar o conteúdo do 1o Item
						...
						aadd(aProd,	{Len(aProd)+1,;
							...
							(cAliasSD2)->D2_PRCVEN,;//  SATO - 14/01/2020 - INCLUÍDO D2_PRCVEN    // O valor unitario sera obtido pela divisao do valor do produto pela quantidade comercial de acordo com o  Manual do Contribuinte 6.00 realizado na tag <vUnCom>(ConvType(aProd[10]/aProd[09],21,8)) 
		...
		Else				
			...
			While !Eof() .And. xFilial("SD1") == (cAliasSD1)->D1_FILIAL .And.;
		       ...
		       EndIf   		
				...
				cTPNota := If( Empty(cTPNota) , NfeTpNota(aNota,aNfVinc,cVerAmb,aNfVincRur,aRefECF,(cAliasSD1)->D1_CF), cTPNota)   // SATO - 14/10/2021 - Ronilton - Pegar o conteúdo do 1o Item
...
If FunName() <> "SPEDNFSE"
	IF cVeramb >= "4.00"
	   //Ajute para alimentar o aDetPag
		cTPNota := If( Empty(cTPNota) , NfeTpNota(aNota,aNfVinc,cVerAmb,aNfVincRur,aRefECF,aProd[1,7]), cTPNota)   // SATO - 14/10/2021 - Ronilton - Pegar o conteúdo do 1o Item
...
//  SATO - 14/01/2020 - INCLUÍDA TRANSMISSÃO DO D2_PRCVEN
cString += '<vUnCom>'+ IIf(cF2Tipo == "C" .and. cTipoCompl <> '2' ,ComplPreco(cTipo,cF2Tipo,aProd),ConvType(aProd[16],21,8))+'</vUnCom>'
...
//  SATO - 14/01/2020 - INCLUÍDA TRANSMISSÃO DO D2_PRCVEN
cString += '<vUnTrib>'+ IIf(cF2Tipo == "C" .and. cTipoCompl <> '2' ,ComplPreco(cTipo,cF2Tipo,aProd),ConvType(aProd[16],21,8))+'</vUnTrib>'	
